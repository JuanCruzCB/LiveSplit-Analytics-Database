...
SHEET_URL = "https://docs.google.com/spreadsheets/d/1q1e9GCgaUc-LbhQWHEVjKkl0275hkfDVq0rHgQLrF-E/edit?usp=sharing"
...
def get_days_hours_str(seconds: int) -> str:
    days = seconds // 86400
...
def open_sheet() -> Spreadsheet:
    ...
    spreadsheet = client.open_by_url(url=SHEET_URL)
    return spreadsheet


def copy_excel_to_sheet(
    spreadsheet: Spreadsheet, sheet_tab_name: str, excel: DataFrame, range_name: str
) -> None:
    ...
    try:
        original_sheet = spreadsheet.worksheet(title=sheet_tab_name)
        if sheet_tab_name != "General" and sheet_tab_name != "Resets":
            old_sheet = spreadsheet.worksheet(title=old_sheet_tab_name)
        ...
        raise Exception(f"The tab {sheet_tab_name} does not exist in the google sheet.")
...
@measure_time
def copy_doorsplits_to_sheet(spreadsheet: Spreadsheet, doorsplits: Path) -> None:
...
def copy_chapters_to_sheet(
    spreadsheet: Spreadsheet, chapters: Path, chapters_by_doors: Path
) -> None:
...
def copy_general_stats_to_sheet(
    ...
    for i, seconds in enumerate(total_playtime_row[1:]):
        try:
            days_hours_str = get_days_hours_str(int(seconds))
...
def copy_graphs_to_sheet(
    ...
    try:
        sheet = spreadsheet.worksheet(title=SHEET_TAB_NAME)
